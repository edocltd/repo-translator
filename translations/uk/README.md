...
> **Створено на реальному досвіді**: Кожна функція існує через реальну проблему, виявлену при перекладі [claude-howto](https://github.com/luongnv89/claude-howto) (22K+ ⭐) українською. Див. [docs/failure-analysis.md](../../docs/failure-analysis.md) для повного аналізу 48 проблем.
...
SCAN  →  TRANSLATE  →  VALIDATE  →  FIX  →  SYNC
  ↓         ↓            ↓          ↓        ↓
"Які      "Будь-який  "Чи         "Авто-   "Що
 файли?"   метод"     правильно?" фікс"   застаріло?"
```
